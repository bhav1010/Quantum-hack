...
def jones_vector(polarization: str, phase_deg: float = 0.0) -> np.ndarray:
    ...
    states = {
        ...
        'elliptical': np.array([np.cos(np.pi / 4),
                                np.sin(np.pi / 4) * np.exp(1j * ph)]),
    }
...
def waveplate_matrix(phase_deg: float, fast_axis_deg: float = 0.0) -> np.ndarray:
    ...
    """
    Γ = np.deg2rad(phase_deg)
    α = np.deg2rad(fast_axis_deg)
    c, s = np.cos(α), np.sin(α)
    # Rotation matrix
    R  = np.array([[c, s], [-s, c]], dtype=complex)
    Ri = np.array([[c, -s], [s, c]], dtype=complex)
    # Phase matrix (fast axis along x)
    P  = np.array([[np.exp(-1j * Γ / 2), 0],
                   [0,  np.exp( 1j * Γ / 2)]], dtype=complex)
    return Ri @ P @ R
...
def animate_polarization(E: np.ndarray, n_frames: int = 300) -> plt.Figure:
    ...
    Ex = Ex_amp * np.cos(t + δx)
    Ey = Ey_amp * np.cos(t + δy)
...
def stokes_sphere_projection(stokes: dict) -> plt.Figure:
    ...
    θ_p = np.linspace(0, 2 * np.pi, 200)
    axes[1].plot(np.cos(θ_p), np.sin(θ_p), color=GRID, lw=1)
...
def animated_pipeline_html(E_stages: list, use_qwp: bool, use_hwp: bool, intensity: float) -> str:
    """
    ...
    def stage_params(E):
        ...
        dX  = float(np.angle(E[0]))
        dY  = float(np.angle(E[1]))
...
function drawEllipseMini(cx, cy, s, active) {{
  ...
  ctx.beginPath();
  for (let i = 0; i <= 120; i++) {{
    const tt = i / 120 * Math.PI * 2;
    const ex = cx + s.ExA * R * Math.cos(tt + s.dX);
    const ey = cy - s.EyA * R * Math.cos(tt + s.dY);
  ...
  // moving dot
  const dotT = t * 0.9;
  const dx = cx + s.ExA * R * Math.cos(dotT + s.dX);
  const dy = cy - s.EyA * R * Math.cos(dotT + s.dY);
...
function drawWaveSegment(x0, x1, midY, s, active, phaseShift) {{
  ...
  if (s.ExA > 0.01) {{
    ...
    for (let i = 0; i <= steps; i++) {{
      ...
      const wt = (t + phaseShift) - (i/steps) * Math.PI * 4;
      const yp = midY - amp * s.ExA * Math.cos(wt + s.dX);
  ...
  if (s.EyA > 0.02) {{
    ...
    for (let i = 0; i <= steps; i++) {{
      const xp = x0 + i/steps * W;
      const wt = (t + phaseShift) - (i/steps) * Math.PI * 4;
      const yp = midY - amp * s.EyA * Math.cos(wt + s.dY);
      i === 0 ? ctx.moveTo(xp, yp) : ctx.lineTo(xp, yp);
    }}
    ctx.stroke();
    ctx.restore();
  ...
  if (s.ExA > 0.01) {{
    const prog = ((t * 0.5) % (Math.PI * 2)) / (Math.PI * 2);
    const dotX = x0 + prog * W;
    const wt2 = (t) - prog * Math.PI * 4;
    const dotY = midY - amp * s.ExA * Math.cos(wt2 + s.dX);
    ctx.save();
...
function drawElement(x, y, w, h, label, color, active) {{
  ctx.save();
  ...
  if (label === 'LASER') {{
    ...
    // detector square with fill proportional to intensity
    roundRect(ctx, cx-14, cy-8, 28, 22, 3);
    ctx.stroke();
    ctx.fillStyle = color;
    ctx.globalAlpha = Math.max(0.05, intensity) * (active ? 1 : 0.2);
    ctx.fill();
...
function frame() {{
  ...
  for (let i = 0; i < 5; i++) {{
    ...
    drawWaveSegment(x0, x1, midY, s, active, i * 0.4);
  }}
...
params = [
    ...
    (c2, "S₀ (Total)", f"{stokes['S0']:.4f}", "total power"),
    (c3, "S₁ (H/V)",   f"{stokes['S1']:+.4f}", "linear H-V"),
    (c4, "S₂ (Diag)",  f"{stokes['S2']:+.4f}", "linear ±45°"),
    (c5, "S₃ (Circ)",  f"{stokes['S3']:+.4f}", "circular"),
]
...
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("---")
st.markdown("## 🎤 1-Minute Hackathon Demo Script")

st.markdown("""
<div class="script-box">
<span class="script-time">[0:00–0:12]</span> — <b>Intro</b><br>
"This is our Polarization Optics Visual Lab. We simulate the full optical pipeline —
Laser, Polarizer, Wave Plates, Analyzer, Sensor — using Jones calculus, and we map
every simulation parameter to real optical hardware."<br><br>

<span class="script-time">[0:12–0:28]</span> — <b>Linear → Circular with QWP</b><br>
"Start with horizontal linear light (0°). Activate the QWP toggle. Watch the
polarization ellipse transform from a flat line into a circle — that's left circular
polarization. The intensity curve flattens because circular light has no preferred
direction for the analyzer. In hardware, this is a real quarter-wave plate."<br><br>

<span class="script-time">[0:28–0:42]</span> — <b>HWP Rotation</b><br>
"Now activate the HWP instead. The half-wave plate rotates our polarization axis by
twice the fast-axis angle. Watch the cos² curve shift right on the intensity graph —
exactly as Malus' Law predicts. This maps directly to rotating a real HWP mount."<br><br>

<span class="script-time">[0:42–0:55]</span> — <b>Malus Law in Hardware Mode</b><br>
"Switch to Hardware Mode. You can see all three behaviours overlaid: no plate gives
classic cos², QWP gives near-constant output, HWP shifts the peak. Stokes parameters
update live and project onto the Poincaré sphere."<br><br>

<span class="script-time">[0:55–1:00]</span> — <b>Close</b><br>
"Every slider maps to a physical optical element. This is a live physics engine for
optical polarimetry — useful for teaching, experiment planning, and hardware validation."
</div>
""", unsafe_allow_html=True)
